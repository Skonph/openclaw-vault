...
def _save_pending_orders(data: dict):
    f = VAULT_DIR / 'OpenClaw/pending_orders.json'
    try:
        f.write_text(json.dumps(data, indent=2))
    except Exception as e:
        print(f'⚠️  save pending orders: {e}')


def _verify_fills():
    """
    Cross-check 'executed' orders in pending_orders.json against Alpaca.
    If Alpaca shows expired/cancelled, correct the status and send Telegram alert.
    Also records fill quality (submitted_limit vs actual filled_avg_price).
    """
    data    = _load_pending_orders()
    ...
    for order in orders:
        ...
        if not alpaca_id or alpaca_id == 'unknown':
            continue

        alpaca_order = alpaca_get(f'/orders/{alpaca_id}')
        if not alpaca_order:
            continue

        alpaca_status = alpaca_order.get('status', '')

        # ── Fill mismatch: order didn't actually fill ─────────────────────────
        if alpaca_status in ('expired', 'cancelled', 'canceled', 'rejected'):
            order['status']       = 'unfilled'
            order['unfill_reason'] = f'Alpaca: {alpaca_status}'
            changed += 1
            send_telegram(
                f'⚠️ *Fill mismatch: [{order["trade_id"]}] {order["symbol"]}*\n'
                f'Marked as executed but Alpaca shows: `{alpaca_status}`\n'
                f'_Status corrected to unfilled. Will retry next scan._'
            )
            print(f'  ⚠️  Fill mismatch fixed: [{order["trade_id"]}] → unfilled')
            continue

        # ── Fill quality: compare actual fill vs submitted limit ──────────────
        filled_avg = alpaca_order.get('filled_avg_price')
        submitted  = order.get('submitted_limit')
        if filled_avg and submitted and alpaca_status == 'filled':
            slippage = round(float(filled_avg) - float(submitted), 3)
            order['filled_avg_price'] = float(filled_avg)
            order['fill_slippage']    = slippage
            changed += 1

    if changed:
        _save_pending_orders(data)
        print(f'✅ Fill verification: {changed} order(s) updated')
    else:
        print('✅ Fill verification: all orders consistent')


def _pnl_summary() -> str:
    """Build cumulative realized P&L summary from pnl_history."""
    data    = _load_pending_orders()
    ...
    lines = [
        f'📊 *Realized P&L — All Time*',
        f'{icon} Total: *${total_pnl:+.2f}* | Trades: {len(history)} | '
        f'Win rate: {win_rate:.0f}%',
        f'Avg win: ${avg_win:+.2f} | Avg loss: ${avg_loss:+.2f}',
        '',
    ]

    recent = sorted(history, key=lambda x: x['timestamp'], reverse=True)[:3]
    if recent:
        lines.append('_Recent closes:_')
        for h in recent:
            r_icon = '✅' if h['pnl'] > 0 else '❌'
            lines.append(
                f'{r_icon} `{h["symbol"]}` ${h["pnl"]:+.2f} '
                f'— {h["reason"]} ({h["timestamp"][:10]})'
            )
...
def run():
    ...
    if not ALPACA_KEY or not ALPACA_SECRET:
        print('❌ Alpaca credentials missing — check /home/ubuntu/openclaw/.env')
        return

    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print('❌ Telegram credentials missing — check /home/ubuntu/openclaw/.env')
        return

    # Fill verification runs silently before report (fixes any mismatches)
