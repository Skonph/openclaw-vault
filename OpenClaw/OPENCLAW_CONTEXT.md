...
## 10. Current Account State (As of 2026-06-06)

- **Equity:** $2,898.17 (paper)
- **Open positions:** None
- **Pending from June 5 scan:**
  - `CZR` Bull Call — skipped, `events_status: uncertain` → Hermes resolves Monday
  - `GAP` Bear Put — skipped, `events_status: uncertain` → Hermes resolves Monday

---

## 11. Recent Changes (2026-06-08 session)

1. **IC 4-leg executor fully operational — `nova_executor.py`**
   - Added root `'qty'` support for multi-leg submissions to fix Alpaca's `qty must be > 0` 422 error.
   - Updated `cmd_execute` output logging and terminal printouts to format all 4 legs for Iron Condors.
   - Added server-side fallback paths (`/home/ubuntu/openclaw-vault/...`) so execution scripts run smoothly on both Mac and the remote server.

2. **TOST Iron Condor executed manually**
   - Manually checked and verified calendar (TOST earnings are estimated for early August, no ex-div).
   - Executed order `DD5B11B6` with Qty 1 at a `$0.54` limit credit.
   - Successfully updated and pushed `pending_orders.json` to GitHub.

3. **Server Git Authentication Fixed**
   - Configured a fine-grained GitHub PAT (`openclaw-vault-server`) on the Ubuntu server. Pushes and pulls now authenticate seamlessly without 403 authorization errors.
...
|-------|----------|--------|
| `approval_manager.py approve` may not set `events_status → clear` | 🟡 Medium | **Monitor** — need to ensure Hermes changes this value when auto-approving. |
| position_monitor double-fire at 21:20 (via vault_updater) + 21:30 (cron) | 🟡 Medium | Likely Tradier's monitor, not OpenClaw's — confirm path |
...
| Hermes has 10 min (21:10–21:20) to resolve all UNCERTAIN tickers | 🟡 Medium | Fine for 1–2 tickers. If 4+ UNCERTAIN, vault_updater may fire before Hermes finishes. |
