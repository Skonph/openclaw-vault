...
# Try standard server path first, fallback to sibling/parent for local development
if os.path.exists('/home/ubuntu/openclaw-vault'):
    VAULT_DIR = Path('/home/ubuntu/openclaw-vault')
else:
    # Local fallback (SkonVault is parent of openclaw)
...
def mark_approved(trade_id: str, approved_by: str = 'Skon') -> bool:
    ...
    for order in data['orders']:
        if order['trade_id'].upper() == trade_id.upper():
            ...
            order['events_status'] = 'clear'
            order['approved_by'] = approved_by
            order['approved_at'] = _now()
            _save(data)
            print(f'✅ Order {trade_id} approved — ready for execution (events cleared)')
            return True
