...
Reads Alpaca credentials from trading-bot/.env.local (fill in once, never commit).
...
def _build_payload(order: dict) -> dict:
    ...
    # Iron Condor requires 4-leg execution — not yet implemented
    if spread_type == 'iron_condor':
        raise NotImplementedError(
            f"Iron Condor [{order['trade_id']}] requires 4-leg executor update — "
            f"cannot submit yet.\n"
            f"  Put spread:  ${order.get('put_long_strike')} / ${order.get('put_short_strike')}\n"
            f"  Call spread: ${order.get('call_short_strike')} / ${order.get('call_long_strike')}\n"
            f"  Use the Approval Dashboard to hold until executor is updated."
        )
...
def cmd_dry_run(trade_id: str):
    ...
    print(f'  Limit    : ${payload["limit_price"]}  (spread_mid × 1.05)')
...
def cmd_execute(trade_id: str):
    ...
    if r.status_code in (200, 201):
        ...
        log_entry = (
            ...
            f'- **Debit**: ${order["spread_mid"]:.2f}  R:R: {order["rr"]:.1f}x\n'
